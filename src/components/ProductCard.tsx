...
const truncate = (name: string) => name.length > 40 ? name.slice(0, 40) + "..." : name
...
const ProductCard = ({ product }: {product: ProductT}) => {
  ...
  return (
    <article className='border h-80 w-56 py-2 px-4 flex flex-col justify-between'>
      <div>
          <div className='h-40'>
        <img src={product.image} alt={product.title + 'image'} className='w-full h-32 object-contain object-top' />
        </div>
        <div>
        ...
        <Link to={`product/${product.id}`} className='bg-black text-white px-4 py-2'>Buy</Link>
